#!/usr/bin/env sh
# Source https://stepan.wtf/importing-protobuf-with-go-modules/
# set -emou pipefail
...
go list -f "${PROTOBUF_IMPORT_DIR}/{{ .Path }}" -m all | grep go-grpc-std \
  | xargs -L1 dirname | sort | uniq | xargs mkdir -p

# Create symlinks
go list -f "{{ .Dir }} ${PROTOBUF_IMPORT_DIR}/{{ .Path }}" -m all | grep go-grpc-std \
  | xargs -L1 -- ln -s